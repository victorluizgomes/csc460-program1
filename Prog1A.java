...
/*=============================================================================
|   Assignment:  Program #1 (Prog1A.java)
...
public class Prog1A {
	public static void main(String[] args) {
		...
		TreeMap<Date, ArrayList<String>> tmap = new TreeMap<Date, ArrayList<String>>();
		...
		try {
			...
			while((currentLine = buffR.readLine()) != null) {
				...
				if(tmap.get(date) == null) {
					...
					ArrayList<String> arr = tmap.get(date);
					arr.add(currentLine);
					tmap.put(date, arr);
				}
			}
...
class AllDataRecords {
    ...
    *-------------------------------------------------------------------*/
	public void readBinary(RandomAccessFile stream) {
		...
		// System.out.println("READ BINARY STUFF: "); ERASE TODO SYSTEM>OUT
